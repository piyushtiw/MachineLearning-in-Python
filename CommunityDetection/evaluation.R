...
# ===== Reading the graph ====== #
g <- read.graph(file="ncsu/adbi/CommunityDetection/data/fb_caltech_small_edgelist.txt", format = c("edgelist"))

# ===== Read communities file ==== #
comm = integer(vcount(g))

conn <- file("ncsu/adbi/CommunityDetection/communities_1.txt", open="r")
...
attrData <- read.csv("ncsu/adbi/CommunityDetection/data/fb_caltech_small_attrlist.csv")
