...
def records_to_graph():
    ...
    for movie in movies:
      create_node(movie, 'movie')
      for rating in rating_range:
        create_node(movie +"_" +rating, 'movie-rating')
      
    [create_node(director, 'director') for director in directors]

    [create_node(actor, 'actor') for actor in actors]

    [create_node(genre, 'genre') for genre in genres]

    [create_node(user, 'user') for user in ratings]

    # Add edges between users and movie-rating nodes
    # YOUR CODE HERE

    for user in ratings:
      user_rated_movies = ratings[user]
      user_node = nodedict[user]
      for movie in user_rated_movies:
        rnode = movie + "_" + user_rated_movies[movie]
    ...
    for movie in movies:
      ...
      if (movie_data.director):
        director_node = nodedict[movie_data.director]
