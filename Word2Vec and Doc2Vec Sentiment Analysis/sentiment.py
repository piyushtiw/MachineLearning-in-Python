...
def feature_vecs_NLP(train_pos, train_neg, test_pos, test_neg):
    ...
    for sentence in train_neg:
        for word in set(sentence):
            if word in stopwords:
                continue
            if not word in negative_words:
                negative_words[word] = 1
            else:
                negative_words[word] += 1


    one_percent_pos_texts_len = int(len(train_pos)*0.01);
    one_percent_neg_texts_len = int(len(train_neg)*0.01);
    positive_words_dic = {
        word:frequency for (word, frequency) in positive_words.items() 
        if (frequency >= one_percent_pos_texts_len and (frequency >= 2*negative_words[word] or negative_words[word] >= 2*frequency))
    }
    ngeative_words_dic = {
        word:frequency for (word,frequency) in negative_words.items()
        if frequency >= one_percent_neg_texts_len and (frequency >= 2*positive_words[word] or positive_words[word] >= 2*frequency)
    }

    feature_list = list(set(list(positive_words_dic.keys()) + list(ngeative_words_dic.keys())))

    # Using the above words as features, construct binary vectors for each text in the training and test set.
    # These should be python lists containing 0 and 1 integers.
    # YOUR CODE HERE

    def feature_vector(sentences):
        vector = []
        for sentence in sentences:
            ...
            for word in sentence:
                if word in feature_dict:
                    feature_dict[word] = 1;
            vector.append(list(feature_dict.values()))
        return vector

    train_pos_vec = feature_vector(train_pos)
    train_neg_vec = feature_vector(train_neg)
    test_pos_vec = feature_vector(test_pos)
    test_neg_vec = feature_vector(test_neg)
...
def feature_vecs_DOC(train_pos, train_neg, test_pos, test_neg):
    ...
    labeled_train_pos = [TaggedDocument(s, ["TRAIN_POS_"+str(i)]) for i, s in enumerate(train_pos)]
    labeled_train_neg = [TaggedDocument(s, ["TRAIN_NEG_"+str(i)]) for i, s in enumerate(train_neg)]
    labeled_test_pos = [TaggedDocument(s, ["TEST_POS_"+str(i)]) for i, s in enumerate(test_pos)]
    labeled_test_neg = [TaggedDocument(s, ["TEST_NEG_"+str(i)]) for i, s in enumerate(test_neg)]
    ...
    # YOUR CODE HERE
    train_pos_vec = [model.docvecs[tag] for tag in model.docvecs.doctags.keys() if "TRAIN_POS_" in tag]
    train_neg_vec = [model.docvecs[tag] for tag in model.docvecs.doctags.keys() if "TRAIN_NEG_" in tag]
    test_pos_vec = [model.docvecs[tag] for tag in model.docvecs.doctags.keys() if "TEST_POS_" in tag]
    test_neg_vec = [model.docvecs[tag] for tag in model.docvecs.doctags.keys() if "TEST_NEG_" in tag]
